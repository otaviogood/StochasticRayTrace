...
float Apollonian( vec3 pos )
{
   ...
   //p = clamp(vec3(-1.0,-1.0,-1.0), vec3(1.0,1.0,1.0), p);
	for( int i=0; i<10;i++ )
	{
        ...
        float r3 = pow(abs(pos.x) + abs(pos.y) + abs(pos.z), 1.17);// + RandFloat()*0.001);
        r3 *= r3;
